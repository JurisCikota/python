...
class Game:
    ...
    Create class using one word as atribute,
    this word will be needed to guess.
    ...
    def countdown(self):
        self.a = ''
        self.t = 20
        print("Tev ir dotas 20 sekundes, lai ievadītu burtu!"
    ...
    Print out countdown. countdown can be interupted with
    keyboard key press, when this behaviour happens
    it reads what key has been pressed.
    ...
    def play(self):
        ...
        while self.lives > 0 and self.flag == False:
            ...
            if self.letter in self.try_letters:
                print(f"Šādu burtu {self.letter} Tu jau minēji!"
                      + " Esi uzmanīgāks")
                print("Minētie burti: " + str(self.try_letters))
            elif self.letter == '':
                print(f"\nLaiks ir beidzies! Mēģini iekļauties"
                      + " 20 sekunžu minēšanas laikā!")
                print("Minētie burti: " + str(self.try_letters))
                ...
                if self.letter in str(tr).upper():
                    ...
                    if self.lives == 0:
                        GRleft.printGR(self.lives)
                        print(f"\nVārds <> {self.original_word}"
                              + " <> nav uzminēts, spēle zaudēta!")

            elif len(self.letter) == len(self.word) \
                and self.letter.isalpha():
